...
function notifyListeners(user) {
    authStateListeners.forEach(cb => { try { cb(user); } catch (_) {} });
}
...
export const AuthService = {
    ...
    async login(email, password, rememberMe = false) {
        ...
        currentUser = userData;
        localStorage.setItem('crux_current_user', JSON.stringify(userData));
        if (rememberMe) localStorage.setItem('crux_remember_me', '1');
        notifyListeners(userData);
        return userData;
    },
    ...
    async logout() {
        ...
        localStorage.removeItem('crux_remember_me');
...
export const MeetingService = {
    ...
    async getUserMeetings(userId) {
        const meetings = JSON.parse(localStorage.getItem('crux_meetings') || '[]');
        return meetings
            .filter(m => m.creatorId === userId || (m.participants && m.participants.includes(userId)))
    ...
    async joinMeeting(meetingId, userId) {
        ...
        if (meetings[idx].isLocked) throw new Error('Cette réunion est verrouillée par l\'hôte.');
    ...
    async toggleLock(meetingId) {
        ...
        const idx = meetings.findIndex(m => m.id === meetingId);
        if (idx !== -1) {
            meetings[idx].isLocked = !meetings[idx].isLocked;
            localStorage.setItem('crux_meetings', JSON.stringify(meetings));
            return meetings[idx].isLocked;
        }
        return false;
    },
    ...
    async toggleRecording(meetingId) {
        ...
        const idx = meetings.findIndex(m => m.id === meetingId);
        if (idx !== -1) {
            meetings[idx].isRecording = !meetings[idx].isRecording;
            localStorage.setItem('crux_meetings', JSON.stringify(meetings));
            return meetings[idx].isRecording;
        }
        return false;
    },
    ...
    async saveChatMessage(meetingId, userId, userName, message) {
        ...
        messages.push({ id: generateId(), userId, userName, message, timestamp: new Date().toISOString(), isSystem: false });
